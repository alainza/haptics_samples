...
{
	vr::k_ulInvalidActionHandle,        // ulActionSet
	vr::k_ulInvalidInputValueHandle,    // VRInputValueHandle_t
	vr::k_ulInvalidActionHandle,        // ulSecondaryActionSet
	0,                              // unPadding
	0                              // nPriority
};

// Compute json manifest path
void DeclareActions()
{
	char manifestPath[MAX_PATH];
	GetModuleFileNameA(nullptr, manifestPath, _countof(manifestPath));
	char* file = strrchr(manifestPath, '\\') + 1;
	strcpy_s(file, _countof(manifestPath) - (file - manifestPath), "Hello_actions.json");
	...
	inputError = vr::VRInput()->GetActionSetHandle("/actions/default", &actionSet.ulActionSet);
}
